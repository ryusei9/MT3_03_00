...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		ImGui::DragFloat3("SphereCenter0", &controlPoint[0].center.x, 0.01f);
		ImGui::DragFloat3("SphereCenter1", &controlPoint[1].center.x, 0.01f);
		ImGui::DragFloat3("SphereCenter2", &controlPoint[2].center.x, 0.01f);
		ImGui::End();
		...
		DrawSphere(controlPoint[0], worldViewProjectionMatrix, viewPortMatrix);
		DrawSphere(controlPoint[1], worldViewProjectionMatrix, viewPortMatrix);
		DrawSphere(controlPoint[2], worldViewProjectionMatrix, viewPortMatrix);
...
void DrawSphere(const Sphere & sphere, const Matrix4x4 & viewProjectionMatrix, const Matrix4x4 & viewportMatrix) {
	...
	for (uint32_t latIndex = 0; latIndex < kSubdivision; ++latIndex) {
		...
		for (uint32_t lonIndex = 0; lonIndex < kSubdivision; ++lonIndex) {
			...
			// ab、bcで線を引く
			Novice::DrawLine(int(a.x), int(a.y), int(b.x), int(b.y), 0x000000FF);
			Novice::DrawLine(int(a.x), int(a.y), int(c.x), int(c.y), 0x000000FF);
		}
	}
}
